...
Phase 2 — the PIN gate. Built and tested here, waiting on the SQL and your test.

## Waiting on me (Andrei)

- **Run `supabase/migrations/0002_login_attempts.sql`** in the Supabase SQL editor. Without it, login refuses with "Can't reach the database" — deliberately, so the lockout can never be quietly switched off.
- **Add `PIN_HASH` and `SESSION_SECRET` in Vercel**, ticked for Production, Preview and Development, then redeploy. They're already in `.env.local`.
- Push, then on the phone: open the app, get sent to the PIN screen, type the PIN, land on the home screen. Close and reopen — it should not ask again.
- Approve phase 2.
