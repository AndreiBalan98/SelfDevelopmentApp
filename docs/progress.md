...
- 2026-09-21 — **Phase 7 step 7.14 complete: meals vs snacks, and days on target.**
  ...
  logged, with counts, the day numbers, how many targets each day hit, and the swing
  line (`lib/stats.ts`, `stats-cards.tsx`). Each meal is now added up on its own in
  `mealDaysIn`. Checked against hand-worked numbers, on the phone-sized screen.
- 2026-09-21 — **Phase 7 step 7.13b complete: the stats chart.** Tested on the phone and
  approved. One chart with seven metric buttons (calories, spend, protein, carbs, sugar, fibre,
...
- 2026-09-13 — **Phase 7 step 7.10 complete: the TDEE estimate.** The card under the
  ...
  Rough / Fair / Reliable label, and the Mifflin–St Jeor formula beside it (`lib/tdee.ts`,
  `totalsByDay` in `lib/meals.ts`). Tested on the phone and approved, including the small
  calls under Decisions.
...
- Screens so far: `/login` (the PIN screen, outside the tabs), then everything else
  ...
  chart, and `day` for the form), `/sleep` (the clock, and `night` for the form),
  `/smoking` (the chart, and `day`), `/workout` (the countdown), `/settings` (goal, every target,
...
2026-09-13 — **Step 7.8: how the charts are built.** Each chart is drawn as SVG on the server twice — shaped for the phone upright and for the phone on its side — and `chart-frame.tsx` swaps them: the rotate button lays the sideways drawing across the whole screen turned 90° clockwise, covering the list and the tab bar, with the iPhone's safe areas on the edges that become the top and bottom. Nothing asks the phone to rotate, so it works with the rotation lock on, and the phone downloads no chart code. The range lives in the address (`?range=7`, `?from=…&to=…`), resolved by `lib/range.ts`; the pills are one shared component (`range-control.tsx`) and one shared style (`PILL` in `ui.ts`, which the Products and Recipes sort now uses too). Where things go on a chart is `lib/chart.ts`. Checked in the scratchpad: the arithmetic against hand-worked cases (every range, backwards and over-the-end Custom, the carried-through range refusing anything but a range, date labels across months and years, moving averages with gaps and zeros, axis numbers from 0 to 40, date labels from 1 to 800 days); then the real screen at iPhone 13 and SE sizes against 40 made-up days with a zero, a skipped day and yesterday missing — every pill, Custom typed backwards, turning the chart and back, the "+" opening on yesterday, a refused save keeping the note, saving and deleting returning to the same range, and the red dot on the "+" and the tab icon going and coming back.
2026-09-13 — Step 7.8 small calls: **every range ends yesterday** — today never shows, even if it's been logged early — and Custom can't reach past yesterday. **The moving averages reach back up to six days before the first day shown**, so both lines start where the chart starts; the mockup starts them three and six days in. **Saving or deleting a day goes back to the chart, on the range it was showing** — it used to stay on the form with "Saved 8 for …"; now the new bar and row are the confirmation. **Delete moved from the list to the day's own screen** ("Delete this day"), as the mockup's rows have none. The list shows the logged days only; an unlogged day is the gap on the chart. The entry form is its own screen, `/smoking/day`, titled with the day ("Yesterday, 12 Sep"), with the way back to the chart top right; the old `/smoking?date=…` no longer opens a form. The numbers up the side are 0, round steps and the highest count, a step within the top eighth left out so labels don't collide (0 · 5 · 11). The first and last dates along the bottom are aligned to the chart's edges so they aren't cut off. The key under the chart shows upright only. Picking another range keeps the pills in place and shows a skeleton where the chart was. The form's hint is now "Zero is a real answer — a day of none. A day with nothing typed isn't logged."; the "Last 14 days" heading and the paragraph under the old list are gone, with the averages text and trend sentence the plan removed.
2026-09-13 — Smoking's red dot now sits on the "+" in its header as well as on the tab icon (step 7.8), as the plan wants. Sleep's still waits for 7.11. **Done in 7.11:** Sleep's is on its "+" too.
...
2026-09-21 — **Found by testing 7.14, and the worst kind of bug this project keeps catching:** `judge` in `lib/targets.ts` answers **null** for a target that isn't set, and the first version of Days on target asked `!judge(…)?.red` — which reads null as `undefined`, and `!undefined` is `true`. Every target Andrei hadn't set scored a perfect hit every single day, and the card looked entirely plausible. `lib/stats.ts` now turns null into "not measured" explicitly, and a target with no row at all is the visible result. Caught by a hand-worked case that set only some of the targets. **Anything else built on `judge` must not treat its null as a pass.**
