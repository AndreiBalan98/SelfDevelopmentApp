...
// The search box and the retired toggle on the recipes list. Both just rewrite
// the URL, so the server does the filtering and the result survives a reload.
export function RecipeSearch({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      router.replace(query ? `/recipes?${query}` : "/recipes");
    }, 250);
